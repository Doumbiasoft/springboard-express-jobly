...
const jobSearchQueriesSchema = require("../schemas/jobSearchQueries.json");
...
router.get("/", async function (req, res, next) {
  try {
    const validator = jsonschema.validate(req.query, jobSearchQueriesSchema);
    if (!validator.valid) {
      const errs = validator.errors.map(e => e.stack);
      throw new BadRequestError(errs);
    }
...
router.patch("/:id", ensureLoggedIn, ensureIsAdmin, async function (req, res, next) {
  try {
    ...
    const { id, companyHandle } = req.body
    if (id || companyHandle) {
